...
// 扩充的127个违规关键词权重系统 - 与AI提示词保持一致
const KEYWORD_WEIGHTS = {
    // 地方保护主义类 (30个关键词)
    regional_protectionism: {
        weight: 4,
        terms: ['本地企业优先', '当地供应商', '本地供应商', '本市企业', '本县公司', '本区投资', '当地投资者', '本省企业', '地方龙头', '本土品牌', '当地制造', '本地生产', '当地服务商', '本市供应商', '本县投资', '本区企业', '当地企业优先', '本地化要求', '地方配套', '本土化', '当地参与', '本地合作', '区域协调', '地方统筹', '本地配置', '属地管理', '本市注册', '本地注册', '外地企业', '优先选择']
    },
    // 限定特定经营者类 (30个关键词)
    specific_operator_restriction: {
        weight: 4,
        terms: ['限定经营', '指定供应商', '特定经营者', '必须购买', '只能选择', '排除竞争', '独家经营', '专营权', '特许经营', '指定品牌', '限制品牌', '指定产品', '特定产品', '必须使用', '强制采购', '统一采购', '集中采购', '定点采购', '指定采购', '限制采购', '禁止外购', '不得外购', '限制外购', '指定服务商', '限定服务', '专用产品', '指定设备', '特定技术', '必须委托', '强制委托']
    },
    // 歧视性条件类 (27个关键词) 
    discriminatory_conditions: {
        weight: 3,
        terms: ['歧视性条件', '额外条件', '额外保证金', '额外', '特殊要求', '不同标准', '更高门槛', '附加条件', '特别程序', '额外材料', '特殊审批', '不同程序', '区别对待', '差别政策', '特殊政策', '例外规定', '特别规定', '单独要求', '个别政策', '专门规定', '针对性措施', '区域性政策', '选择性执行', '差异化管理', '分类管理', '区分处理', '特殊待遇']
    },
    // 经济贡献导向类 (22个关键词)
    economic_contribution_based: {
        weight: 3, 
        terms: ['纳税额', '税收贡献', '经济贡献', '产值规模', '营业收入', '利润总额', '就业人数', '投资规模', '固定资产', '年产值', '销售收入', '上缴税金', '创税能力', '经济效益', '社会贡献', '发展贡献', '地方贡献', '财政贡献', '税源建设', '经济指标', '效益指标', '贡献度']
    },
    // 强制限制类 (25个关键词)
    mandatory_restrictions: {
        weight: 4,
        terms: ['强制', '必须', '应当', '不得', '禁止', '限制', '不允许', '不准', '严禁', '杜绝', '制止', '阻止', '限定', '约束', '控制', '规定', '要求', '命令', '指令', '强迫', '逼迫', '迫使', '威胁', '处罚', '制裁']
    }
};

// 18种隐性违规模式识别规则 - 与AI提示词保持一致
const HIDDEN_VIOLATION_PATTERNS = {
    // 规则1-3: 间接地方保护
    indirect_protection: [
        /[本当地].*?企业.*?在.*?(同等条件|相同条件).*?下.*?(优先|倾斜)/g,
        /给予.*?[本当地].*?(企业|投资者).*?(便利|支持|帮助).*?措施/g,
        /[本当地].*?(产品|服务).*?在.*?(政府采购|招标).*?中.*?(加分|优势)/g
    ],
    // 规则4-6: 变相门槛设置 
    disguised_barriers: [
        /要求.*?(投资|注册|经营).*?满.*?\d+.*?(年|月).*?以上/g,
        /要求.*?(年营业额|注册资本|投资额).*?达到.*?\d+.*?以上/g,
        /限制.*?(外[地省市县]|非[本当地]).*?企业.*?参与.*?(特定|某些).*?(项目|活动)/g
    ],
    // 规则7-9: 隐性歧视表述
    implicit_discrimination: [
        /对.*?[本当地].*?以外.*?(企业|经营者).*?实行.*?(特殊|额外).*?(监管|管理)/g,
        /[本当地].*?(企业|经营者).*?可.*?享受.*?(简化|便民).*?(程序|服务)/g,
        /非[本当地].*?(企业|投资者).*?需.*?(提供|出具).*?(额外|特殊).*?(材料|证明)/g
    ],
    // 规则10-12: 经济指标导向
    economic_indicator_bias: [
        /根据.*?(企业|投资者).*?(对当地|对本地).*?经济.*?(贡献|带动).*?给予.*?(政策|支持)/g,
        /按.*?(纳税|投资|产值).*?(规模|数额|贡献).*?分.*?(档次|等级|类别).*?享受.*?政策/g,
        /对.*?(重点|重要|重大).*?企业.*?给予.*?(倾斜|照顾|扶持).*?政策/g
    ],
    // 规则13-15: 市场分割表述
    market_segmentation: [
        /在.*?[本当地].*?(区域|范围).*?内.*?(统一|集中|协调).*?(价格|标准|管理)/g,
        /[本当地].*?(市场|行业).*?实行.*?(统一|集中).*?(管理|监督|协调)/g,
        /建立.*?[本当地].*?(企业|产品|服务).*?(名录|目录|清单|白名单)/g
    ],
    // 规则16-18: 竞争限制暗示
    competition_restriction_hints: [
        /鼓励.*?[本当地].*?(企业|产业).*?(做大|做强|壮大|发展)/g,
        /支持.*?[本当地].*?(龙头|重点|骨干).*?企业.*?(整合|并购|重组)/g,
        /推动.*?[本当地].*?(企业|产业).*?(集聚|集中|一体化).*?发展/g
    ]
};
...
function performPatternAnalysis(text) {
    ...
    // 2. 18种隐性违规模式检测
    for (const [categoryKey, patterns] of Object.entries(HIDDEN_VIOLATION_PATTERNS)) {
        const matches = [];
        ...
        for (const pattern of patterns) {
            ...
            matches.push(...patternMatches.map(match => ({
                text: match[0],
                index: match.index,
                category: `隐性${categoryKey.replace('_', '')}模式`,
                severity: 'high',
                type: 'hidden_pattern'
            })));
        }
    ...
    return {
        ...
        hiddenPatternsDetected: detectedPatterns.filter(p => p.matches.some(m => m.type === 'hidden_pattern')).length
    };
}
